...
학습을 이미 한 객체일까요
학습을 할 객체일까요? 할객체 그러니까 머신러닝인거임.

그리고 알고리즘을 공부하는 목적이
이곳에서(머신러닝에서는)
본인이 이 알고리즘을 공부하는 것이 아니라
내가 만든 러신을 공부시키는 것이다란 개념을 
잡고 있어야 합니다.

이 머신을 신생아(baby) 로 비유할 수 있어요.
그리고 개발자의 평가는 이 baby가 똑똑해져서
...
이제 모델을 만들려고 합니다.
1. model 을 훈련시킬 데이터에서 쓰레기값은 제거하는 겁니다. 
이 머신은 타이타닉의 정보를 통해 만약 A 라는 가상의 인물이 당시 1912년 침몰 당시 
타이타닉에 승선했다면 생존확률은 어찌 될 것인가 ?
더 나아가 현재 승객 안전을 위한 조취를 어떻게 하면 비슷한 사고 발생시
생종률을 올릴 수 있ㅇ르 것인가 하는 문제로 귀결될 수 있습니다. 

우리는 단순하게 생존확률 판단 머신만 만들겠습니다. 

1순위 작업은 feature 을 줄이는 것. 컨텐츠의 지식이 필요함. 판단력. 
